...
static inline void _startTimerParallel(unsigned long long *start){
    struct timeval t;
    gettimeofday(&t, 0);
    *start = (unsigned long long) ((t.tv_sec)*1000000ll) + t.tv_usec;
}

static inline void _stopTimerParallel(unsigned long long *start, float *elapsed){
    struct timeval now;
    gettimeofday(&now, 0);
...
static inline void _stopTimerAddElapsedParallel(unsigned long long *start, float *elapsed){
    struct timeval now;
    gettimeofday(&now, 0);
	unsigned long long nowll = (unsigned long long) ((now.tv_sec)*1000000ll) + now.tv_usec;
	nowll -= *start;
